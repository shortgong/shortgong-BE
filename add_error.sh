...
# 이미 UTF-8 로케일이면 그대로 유지 (깨진 C.UTF-8 로 강제하면 한글 입력이 깨짐)
case "$(locale charmap 2>/dev/null)" in
  UTF-8*) : ;;
  *)
    for loc in ko_KR.UTF-8 ko_KR.utf8 en_US.UTF-8 en_US.utf8 C.UTF-8 C.utf8; do
...
BASE_DIR="src/main/java/com/sungjujjang/shortgong/global/exception"
...
def safe(s):
    # 환경변수에 깨진 바이트가 surrogateescape 로 들어올 수 있음. 원본 바이트로 되돌린 뒤
    # 다시 디코딩해서 서러게이트가 남지 않도록 정규화한다.
    return s.encode('utf-8', 'surrogateescape').decode('utf-8', 'replace')

error_code_file = os.environ["ERROR_CODE_FILE"]
status_code = os.environ["STATUS_CODE"]
error_code = os.environ["ERROR_CODE"]
error_msg = safe(os.environ["ERROR_MSG"])

with open(error_code_file, 'r', encoding='utf-8', errors='surrogateescape') as f:
    content = f.read()
...
output = []
output.append("package com.sungjujjang.shortgong.global.exception;")
output.append("")
...
tmp = error_code_file + '.tmp'
with open(tmp, 'w', encoding='utf-8') as f:
    f.write(new_content)
os.replace(tmp, error_code_file)
